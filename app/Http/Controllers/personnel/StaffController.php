...
class StaffController extends Controller
{
    ...
    {
        $user = $filter->apply(User::where('role','Staff'));
        return $user->get();
    }

    public function create(Request $request)
    {
        $this->staffValidator($request)->validate();
        ...
        try {
            ...
            return response()->json(['message' => 'Unable to create staff account.'], 400);
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        $user = User::find($id);
        if(!$user)
            abort('No such staff in records');
    ...
    {
        $staff = User::find(Hashids::decode($id))->first();
    ...
    public function changeRole(Request $request, $id) {
        $request->validate(['role' => 'required|in:Staff,Reception,Account']);
        $staff = User::find(Hashids::decode($id))->first();
        if(!$staff && ($staff->role != 'Staff' || $staff->role != 'Reception' || $staff->role != 'Account') && !$staff->active)
            abort(404, 'Unable to find person');
        $staff->role = $request->role;
        if($staff->save())
            return response()->json(['message' => 'Success'], 200);
        else
        return response()->json(['message' => 'Unable to role.'], 400);
    }

    private function staffValidator($request)
    {
        $data = [
