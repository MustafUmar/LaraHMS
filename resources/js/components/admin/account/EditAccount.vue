<template>
    <div>
        <page-heading heading="Edit Accountant"></page-heading>
...
export default {
    ...
    mounted() {
        if(_.isEmpty(this.acc)) {
            this.$set(this.err, 'stat', true)
            this.$set(this.err, 'message', 'No such record.')
        } else {
            _.assign(this.form, this.acc)
            this.orig = JSON.parse(JSON.stringify(this.acc))
            this.$set(this.form, 'account',this.acc.active)
        }

    },
    methods: {
        // getStaff() {
        //     axios.get(`api/staff/${this.acc}`).then(response => {
        ...
        submitForm() {
            ...
            if(!this.$v.form.$invalid) {
                ...
                axios.post('api/account/update/'+this.acc.id, fmdata, {headers: { 'Content-Type': 'multipart/form-data' }})
                .then(response => {
                    console.log(response.data)
                    this.loading = false
                    this.$router.replace('/hpl/accounts', () => {
                        this.$root.fireNotifier('Accountant record updated successfully', 'success')
                    })
                }).catch(error => {
                    this.loading = false
                    if(error.response) {
