<template>
    <div>
        <page-heading heading="Edit Staff"></page-heading>
            ...
            <div class="col-md-10 col-sm-11">
                        ...
                        <div class="card-body p-3">
                            <form action="javascript:void(0)" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    <label for="docphone">Job Title</label>
                                    <input type="text" :class="{'form-control':true, 'is-invalid':getError('job_title')}" v-model="form.job_title" placeholder="Job Title">
                                    <div class="invalid-feedback">
                                        <!-- <span v-if="$v.form.job_title.$error">Please enter phone number.</span> -->
                                        <span v-if="getError('job_title')">{{errors.job_title[0]}}</span>
                                    </div>
                                </div>
                                <div class="form-row">
                                    <div class="form-group">
                ...
                <div v-else class="row justify-content-center">
                    <div class="col pd-3 bg-lightgray text-center">
                        ...
                        <p><router-link to="/hpl/staffs" replace class="btn btn-secondary">Back to staffs</router-link></p>
                    </div>
                </div>
...
export default {
    ...
    mounted() {
        if(_.isEmpty(this.stf)) {
            this.$set(this.err, 'stat', true)
            this.$set(this.err, 'message', 'No such record.')
        } else {
            _.assign(this.form, this.stf)
            this.orig = JSON.parse(JSON.stringify(this.stf))
            this.$set(this.form, 'account',this.stf.active)
        }

    },
    methods: {
        // getStaff() {
        //     axios.get(`api/staff/${this.stf}`).then(response => {
        ...
        submitForm() {
            ...
            if(!this.$v.form.$invalid) {
                ...
                axios.post('api/staff/update/'+this.stf.id, fmdata, {headers: { 'Content-Type': 'multipart/form-data' }})
                .then(response => {
                    this.loading = false
                    this.$router.replace('/hpl/staffs', () => {
                        this.$root.fireNotifier('Staff created successfully', 'success')
                    })
                }).catch(error => {
                    this.loading = false
                    if(error.response) {
        ...
        goBack() {
            window.history.length > 1 ? this.$router.go(-1) : this.$router.replace('/hpl/staff')
