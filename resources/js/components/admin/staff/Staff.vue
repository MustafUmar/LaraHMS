<template>
    <div>
        <page-heading heading="Staff"></page-heading>
                ...
                <div v-if="!error.stat">
                    ...
                    <div class="thin-light-border two-flexed-spacebtw mb-2 p-1">
                        <div>
                            ...
                            <router-link  :to="{name:'staff_edit', params:{stf:staff}}" class="btn btn-light"><i class="fas fa-pen"></i> Edit</router-link>
                            <span class="dropdown">
                                ...
                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                    ...
                                    <button class="btn btn-outline-dark dropdown-item" @click="confirmSwap('Reception')"><i class="fas fa-pen"> Reception</i></button>
                                </div>
                            </span>
                        </div>
                    </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-sm-3 order-sm-2">
                                    <div>
                                        <img :src="staff.phurl?staff.phurl:'/images/img-invalid.png'" alt="profile" class="img-thumbnail sc3by4-5"/>
                                    </div>
                                    <hr class="hr"/>
                                    <div class="row">
                                        <div class="col-7">
                                ...
                                <div class="col-sm-9 order-sm-1">
                                    <table class="table">
                                        <tr>
                                            <th>Id</th><td>{{staff.personid}}</td>
                                        </tr>
                                        <tr>
                                            <th>Name</th><td>{{staff.fullname}}</td>
                                        </tr>
                                        <tr>
                                            <th>Email</th><td>{{staff.email}}</td>
                                        </tr>
                                        <tr>
                                            <th>Phone</th><td>{{staff.phone}}</td>
                                        </tr>
                                    </table>
                                </div>
    ...
    methods: {
        getStaff() {
            axios.get(`api/staff/${this.stf}`).then(response => {
                this.$set(this, 'staff', response.data)
                this.loading = false
            }).catch(error => {
                this.loading = true
                this.$set(this.error, 'stat', true)
        ...
        swapRole(rl) {
            this.loading = true
            axios.post(`api/staff/swap/${this.stf}`, {role: rl}).then(response => {
                this.loading = false
                this.confirmed = false
                this.$router.replace({name:'reception', params:{rep: this.stf}}, () => {
        ...
        goBack() {
            window.history.length > 1 ? this.$router.go(-1) : this.$router.replace('/hpl/staffs')
        },
    }
}
</script>
